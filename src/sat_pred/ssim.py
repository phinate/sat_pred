...
np.isclose(
    trim_border(ssim_map1, num_pixels=0), trim_border(ssim_map2, num_pixels=0), atol=1e-05
).mean()
# >> 0.9610714285714286
...
np.isclose(
    trim_border(ssim_map1, num_pixels=5), trim_border(ssim_map2, num_pixels=5), atol=1e-05
).mean()
# >> 1.0
```
"""
...
def create_2d_gaussian_kernel(
    kernel_size: int | list[int], sigma: float | list[float]
) -> torch.Tensor:
...
class SSIM3D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        ux, uy, uxx, uyy, uxy = [
            kernel_outputs[i * batch_size : (i + 1) * batch_size] for i in range(5)
        ]
